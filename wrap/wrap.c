...
kern_return_t wrap_IOConnectCallMethod(mach_port_t connection,   // In
...
kern_return_t wrap_IOConnectCallAsyncMethod(mach_port_t connection,   // In
                                            uint32_t selector,        // In
...
kern_return_t wrap_IOConnectCallStructMethod(mach_port_t connection,   // In
                                             uint32_t selector,        // In
                                             const void *inputStruct,  // In
                                             size_t inputStructCnt,    // In
                                             void *outputStruct,       // Out
                                             size_t *outputStructCntP) // In/Out
{
  return wrap_IOConnectCallMethod(connection, selector, NULL, 0, inputStruct,
                                  inputStructCnt, NULL, NULL, outputStruct,
                                  outputStructCntP);
}

kern_return_t
wrap_IOConnectCallAsyncStructMethod(mach_port_t connection,  // In
                                    uint32_t selector,       // In
                                    mach_port_t wakePort,    // In
                                    uint64_t *reference,     // In
                                    uint32_t referenceCnt,   // In
...
{
  return wrap_IOConnectCallAsyncMethod(
      connection, selector, wakePort, reference, referenceCnt, NULL, 0,
      inputStruct, inputStructCnt, NULL, NULL, outputStruct, outputStructCnt);
}

kern_return_t wrap_IOConnectCallScalarMethod(mach_port_t connection, // In
                                             uint32_t selector,      // In
                                             const uint64_t *input,  // In
                                             uint32_t inputCnt,      // In
                                             uint64_t *output,       // Out
                                             uint32_t *outputCnt)    // In/Out
{
  return wrap_IOConnectCallMethod(connection, selector, input, inputCnt, NULL,
                                  0, output, outputCnt, NULL, NULL);
}

kern_return_t wrap_IOConnectCallAsyncScalarMethod(mach_port_t connection, // In
                                                  uint32_t selector,      // In
                                                  mach_port_t wakePort,   // In
                                                  uint64_t *reference,    // In
                                                  uint32_t referenceCnt,  // In
                                                  ...
                                                  uint32_t *outputCnt) // In/Out
{
  return wrap_IOConnectCallAsyncMethod(connection, selector, wakePort,
                                       reference, referenceCnt, input, inputCnt,
                                       NULL, 0, output, outputCnt, NULL, NULL);
}

kern_return_t wrap_IOConnectSetNotificationPort(io_connect_t connect,
                                                uint32_t type, mach_port_t port,
                                                uintptr_t reference) {
  printf("connect %X, type %X, to notification port %X, with reference %lx\n",
         connect, type, port, reference);
  kern_return_t ret =
      IOConnectSetNotificationPort(connect, type, port, reference);
  printf("return %u\n", ret);
  return ret;
}

kern_return_t wrap_IOSetNotificationPort(mach_port_t connect, uint32_t type,
                                         mach_port_t port) {
  return wrap_IOConnectSetNotificationPort(connect, type, port, 0);
}

IONotificationPortRef wrap_IONotificationPortCreate(mach_port_t masterPort) {
  IONotificationPortRef ref = IONotificationPortCreate(masterPort);
  printf("creating notification port from master %X --> %p\n", masterPort, ref);
  return ref;
}

void wrap_IONotificationPortSetDispatchQueue(IONotificationPortRef notify,
                                             dispatch_queue_t queue) {
  printf("set dispatch queue %p to queue %p\n", notify, queue);
  IONotificationPortSetDispatchQueue(notify, queue);
}

mach_port_t wrap_IODataQueueAllocateNotificationPort() {
  mach_port_t ret = IODataQueueAllocateNotificationPort();
  printf("data queue notif port %X\n", ret);
  return ret;
}

IOReturn wrap_IODataQueueSetNotificationPort(IODataQueueMemory *dataQueue,
...
DYLD_INTERPOSE(wrap_IOConnectCallAsyncStructMethod,
               IOConnectCallAsyncStructMethod);
DYLD_INTERPOSE(wrap_IOConnectCallScalarMethod, IOConnectCallScalarMethod);
DYLD_INTERPOSE(wrap_IOConnectCallAsyncScalarMethod,
...
DYLD_INTERPOSE(wrap_IONotificationPortSetDispatchQueue,
               IONotificationPortSetDispatchQueue);
DYLD_INTERPOSE(wrap_IODataQueueAllocateNotificationPort,
               IODataQueueAllocateNotificationPort);
DYLD_INTERPOSE(wrap_IODataQueueSetNotificationPort,
