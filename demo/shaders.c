...
#define AGX_BLEND 0x09, 0x00, 0x00, 0x04, 0xf0, 0xfc, 0x80, 0x03
 ...
 * shader's uniform_store
  ...
  10: 0521000500c43200     device_load      1, 0, 0, 4, 0, i32, pair, r4_r5,
 r0_r1, 0, signed, lsl 1 18: 0501104500c8f200     device_load      1, 1, 0, 4,
 0, i32, quad, r0_r1_r2_r3, r0_r1, 1, signed, lsl 2 20: 3800 wait             0
...
/*
  ...
  I've seen Mali do this optimization before, but never so aggressively.
   ...
   0: 0501040d00c43200     device_load      1, 0, 0, 4, 0, i32, pair, r0_r1,
  u2_u3, 0, signed, lsl 1 8: 3800                 wait             0 a:
  be890a042c00         convert          u32_to_f, $r2, r0.discard, 1 10:
  be810a242c00         convert          u32_to_f, $r0, r1.discard, 1 16:
  9a85c4020200         fmul             $r1, r2.discard, 0.5 1c: 0a05c282 rcp
  r1, r1.discard 20: 9a81c0020200         fmul             $r0, r0.discard, 0.5
...
uint8_t vertex_pre[] = {
    0x05, 0x01, 0x04, 0x0d, 0x00, 0xc4, 0x32, 0x00, 0x38, 0x00, 0xbe,    0x89,
    0x0a, 0x04, 0x2c, 0x00, 0xbe, 0x81, 0x0a, 0x24, 0x2c, 0x00, 0x9a,    0x85,
    0xc4, 0x02, 0x02, 0x00, 0x0a, 0x05, 0xc2, 0x82, 0x9a, 0x81, 0xc0,    0x02,
    0x02, 0x00, 0x0a, 0x01, 0xc0, 0x82, 0xc5, 0x08, 0x80, 0x3d, 0x00,    0x80,
    0x30, 0x00, 0xc5, 0x00, 0xa0, 0x3d, 0x00, 0x80, 0x30, 0x00, AGX_STOP};
...
uint8_t clear[] = {0x7e, 0x01, 0x8c, 0x09, 0x80, 0x40,      0x7e,
                   0x05, 0x8e, 0x09, 0x80, 0x00, AGX_BLEND, AGX_STOP};

uint8_t frag_aux3[] = {0x7e, 0x00, 0x04, 0x09, 0x80, 0x00, 0xb1, 0x80,    0x00,
                       0x80, 0x00, 0x4a, 0x00, 0x00, 0x0a, 0x00, AGX_STOP};

uint32_t demo_upload_shader(const char *label, struct agx_allocator *allocator,
...
#if 1
  return demo_upload_shader("fragtex", allocator, fragment_shader,
...
uint32_t demo_vertex_pre(struct agx_allocator *allocator) {
  return demo_upload_shader("vertex_pre", allocator, vertex_pre,
                            sizeof(vertex_pre));
}
...
uint32_t demo_frag_aux3(struct agx_allocator *allocator) {
  return demo_upload_shader("frag_aux3", allocator, frag_aux3,
                            sizeof(frag_aux3));
}
